...
(function(){
  ...
  function statusLabel(status){
    ...
    const labels={open:'Open',user_replied:'User Replied',admin_replied:'Admin Replied',resolved:'Resolved',closed:'Closed'};
    return labels[value]||value.replace(/_/g,' ').replace(/\w/g,c=>c.toUpperCase());
  }
  function statusClass(status){return 'status-'+String(status||'open').toLowerCase().replace(/[^a-z0-9_-]/g,'_');}
  function isClosedStatus(status){return ['resolved','closed'].includes(String(status||'').toLowerCase());}
  function filterRowsByStatus(rows,current){
    const filter=String(current||'active').toLowerCase();
    if(filter==='all') return rows;
    if(filter==='active') return rows.filter(t=>!isClosedStatus(t.status));
    return rows.filter(t=>String(t.status||'open').toLowerCase()===filter);
  }
  function filterBox(current){
    const c=String(current||'active').toLowerCase();
    const opts=[['active','Active / Open'],['open','Open'],['user_replied','User Replied'],['admin_replied','Admin Replied'],['resolved','Resolved'],['closed','Closed'],['all','All Tickets']];
    return `<label class="mobile-ticket-filter">Status <select id="mobileHelpdeskStatusFilter">${opts.map(o=>`<option value="${o[0]}" ${o[0]===c?'selected':''}>${o[1]}</option>`).join('')}</select></label>`;
  }
  async function statusFixFromMessages(rows){
    return (rows||[]).map(t=>({ ...t, status: t.status || 'open' }));
  }

  async function tickets(uid){const {data,error}=await sb().from(TABLE).select('*').eq('created_by',uid).order('updated_at',{ascending:false}); if(error)return alert(error.message); const rows=await statusFixFromMessages(data||[]); const current=(document.getElementById('mobileHelpdeskStatusFilter')?.value)||'active'; const shown=filterRowsByStatus(rows,current); const el=sheet(`<div class="mobile-helpdesk-card"><header><strong>My Help Desk Tickets</strong><button type="button" data-close>×</button></header><div>${filterBox(current)}${shown.length?shown.map(t=>`<button class="mobile-ticket ${statusClass(t.status)}" type="button" data-id="${esc(t.id)}"><b>${esc(t.type)} - ${esc(statusLabel(t.status))}</b><span>Status: ${esc(statusLabel(t.status))}</span><span>${esc(t.related_tour_label||'No tour')}</span><small>${esc((t.description||'').slice(0,70))}</small></button>`).join(''):`<p>No ${current==='active'?'active / open':current} tickets.</p>`}</div></div>`); el.querySelector('#mobileHelpdeskStatusFilter')?.addEventListener('change',()=>tickets(uid)); el.querySelectorAll('[data-id]').forEach(b=>b.onclick=()=>ticketDetail(b.dataset.id,uid));}
  async function ticketDetail(id,uid=null){const u=uid?{id:uid}:await currentUser(); if(!u)return; const {data:t,error:te}=await sb().from(TABLE).select('*').eq('id',id).single(); if(te)return alert(te.message); const {data:msgs,error:me}=await sb().from(MSGS).select('*').eq('ticket_id',id).order('created_at',{ascending:true}); if(me)return alert(me.message); const forced=(msgs||[]).some(m=>String(m.sender_role||'').toLowerCase()==='admin' && String(m.message||'').toLowerCase().includes('closed by admin'))?'closed':((msgs||[]).some(m=>String(m.sender_role||'').toLowerCase()==='admin' && String(m.message||'').toLowerCase().includes('marked resolved by admin'))?'resolved':null); if(forced && !isClosedStatus(t.status)){ t.status=forced; try{ await sb().from(TABLE).update({status:forced,resolved_at:new Date().toISOString()}).eq('id',id); }catch(_e){} } await sb().from(TABLE).update({user_unread:false}).eq('id',id); await badge(u.id); const el=sheet(`<div class="mobile-helpdesk-card"><header><strong>${esc(t.type)} Ticket</strong><button type="button" data-close>×</button></header><p><b>Status:</b> ${esc(statusLabel(t.status))}</p><p><b>Tour:</b> ${esc(t.related_tour_label||'None')}</p>${(msgs||[]).map(m=>`<div class="mobile-help-msg"><b>${esc(m.sender_role)}</b><p>${esc(m.message)}</p></div>`).join('')}${isClosedStatus(t.status)?'<p class="muted">This ticket is closed/resolved.</p>':`<form id="mobileReplyForm"><label>Reply<textarea name="message" required></textarea></label><button class="btn full" type="submit">Reply</button></form>`}</div>`); el.querySelector('#mobileReplyForm')?.addEventListener('submit',async(e)=>{e.preventDefault(); const msg=new FormData(e.target).get('message'); const {error}=await sb().from(MSGS).insert({ticket_id:id,sender_id:u.id,sender_role:'user',message:msg}); if(error)return alert(error.message); await sb().from(TABLE).update({status:'user_replied',admin_unread:true,user_unread:false}).eq('id',id); ticketDetail(id,u.id);});}
